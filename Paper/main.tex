...
\setcounter{page}{1}
	...
	Projectile motion is the motion in which an object travels under the constant effect of a force field. The simplest form of projectile motion considers a closed system with no air resistance while a more advanced one might consider multiple gravitational pulls or air resistance \todo{Du skulle kunna säga typ friction forces (istället för air resistance) om du vill hålla det mer generellt}. This paper will first describe the ideal trajectory and properties of a projectile parabola\todo{Här introcerar du projectile parabola lite innan du sagt vad det är, men förstår om du vill göra det} and then develop and test an analytical solution to quadratic air resistance \todo{Kanske borde vara typ "and then progressing to models of quadratic air resistance"}.
	\section{Trajectory}
	Consider a Euclidean space $\mathbb{R}^3$ with the three axes $x \comma y$ and $z$. At any time $t$ we want to be able to describe the acceleration, velocity and position of an object with the three vectors $A = (a_x \comma a_y \comma a_z)\comma V = (v_x \comma v_y \comma v_z)$ and $P = (x \comma y \comma z)$. Let $\theta$ be the angle between the $xz$ plane and the initial direction of the object and $\alpha$ the angle between the $x$ axis and a projection of the path on the $xz$ plane. This is visualized in the graph \todo{figure?} below.
	\begin{center}
		\trimbox{0cm 0cm 0cm -0.6cm} {
			...
			vector guide/.style={dashed, black, ultra thick}]
	...
	\end{center}
	\todo{Den här är ju fett snygg och ger ett bra intryck för papret}
	...
	\begin{align}\nonumber
		\int-g\,dt = -gt + C
	\end{align}
	Here $C$ is a constant of integration and can be written as $v_0 \sin\theta$ due to $\theta$ being the angle between the $xz$ plane and the $y$ axis. \todo{Känns lite omotiverat} This allows us to define the total velocity vector $V$.
	\begin{align}\nonumber
		V=
		\begin{bmatrix} 
	...
	\end{align}
	Which is a second degree polynomial \todo{Kanske räcker med att sluta meningen här} proving that projectile motion can be described as a parabola.
	...
	\end{align}
	Here $y_{max}$ denotes the highest point the object will reach when thrown at the angle $\theta$ and at the speed $v_0$. This relationship can also be derived using the conservation of energy.\todo{Här är mindre rigoröst (du sa liksom F=ma förut), men fattar definitivt varför} \todo{Dock nice att du tar med det också och visade att härledningen kan göras mycket lättare}
	\begin{align}\nonumber
	...
	\end{align}
	\todo{Du trollar bort lösningen t=0 när du delar med t, men förstår varför}
	...
	\todo{Kanske typ "The the optimal theta for maximizing R may be determined by studying the derivative of R with respect to theta." Så att det blir lite klarare att du vill optimera theta }
	...
	\todo{Är det här inte angle at a given time?}
	At any given time $t$ we are moving in the $x$, $y$ and $z$ directions at a velocity $v$ which is a combination of $v_x$, $v_y$ and $v_z$. We can determine the absolute velocity $||v||$ using the Pythagorean theorem for three dimensions.\todo{Här nämner du pyth men inte när du räknar ut R}
	\begin{align}\nonumber
	...
	For the purpose of this section we will not consider the $z$ axis nor the angle $\alpha$ between the $x$ and $z$ axis. The reasoning behind this is that is would obstruct \todo{Skulle det verkligen obstruct? Blir det inte bara svårare att se? typ introduce unnecessary complexity } the results we're trying to conclude. Know that these results are applicable \todo{kanske expandable eller may be expanded} to three dimensions with minimal adjustments \todo{Kanske for example istället} including but not limited to multiplying uses of $x$ with $\cos\alpha$.
	
	\subsection{Specific projectile range}
	\todo{På vilket sätt specifyar du projectile range i den här sektionen?}
	...
	\end{align}
	\todo{Känns lite konstigt att theta och v0 är funktioner av konstanter. Känns som att man kan använda funktionerna ovan för att räkna ut ex theta vid en viss tidpunkt, men du vill ju bara att man ska kunna räkna ut theta om man har R och v0 (antar jag)}
	...
	\end{align}
	\todo{Ska det inte bara vara x och y? Du har definierat P = (x, y). Du skriver ju ändå om till x och y sen}
	and we want to determine $\theta$ such that $P_x = x$ and $P_y = y$ where $P$ is the total position vector and $v_0$ and $t$ is the same for both $P_x$ and $P_y$ \todo{Behöver du berätta det här?}. Since $P_y$ is the more convoluted expression we solve $P_x$ for $t$ and substitute it into $P_y$.
	\begin{align}\nonumber
	...
	\end{align}
	Setting the second term as negative yields a negative time when $y_0 > 0$ which is nonsense \todo{Which is not defined eller physically impossible? (nonsense är bra annars)}. We will therefore write it as positive from now on. \todo{kan bindas ihop med förra "... and can/may now find the length"}We can now find the length of a parabola \todo{observing how it varies with...?} and see how it varies with a changing angle $\theta$. Let $R$ denote the range of the projectile and let it be given by $x(t_{max})$. \todo{Kanske bara " given by x(tmax)"}
	\begin{align}\nonumber
	...
	\todo{Kanske borde säga varför det ska göras numeriskt: because this can not be solved by analytical methods, it must be solved numerically. For this solution we want to write an algorithm...}
	We want to write an algorithm that takes the initial height and velocity and returns the optimal angle. The code tests all possible angles $\theta$ \todo{Du kan ju inte testa alla} in our equation for $R$ and determines which gives the farthest possible throw. The following pseudo code details the procedure.\todo{The procedure is described in the following pseudo code (kanske)}
	\begin{algorithm}[H]
		...
		\end{algorithmic}
	\end{algorithm}
	The following two plots are the results of the above algorithm with the initial conditions $v_0 = 10\,m/s$, $h_{min} = 0\,m, h_{max} = 20\,m$ and $v_0 = 25\,m/s$, $h_{min} = 0\,m, h_{max} = 100\,m$ respectively.\todo{Kanske inte behöver säga skalan eftersom det framgår i figuren. Kanske bara typ "is shown in the following two plots"}
	\begin{center}
		\begin{tikzpicture}
		\begin{axis}[ 
	...
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	These graph both follow the same pattern that the distance gained from aiming upwards shrinks as the starting height increases. \todo{Du menar derivatan eller? Kanske borde säga att det är så man ser det} This is because the travel time cannot be changed all that much when the starting height already contributes so much to the distance.
	\\~\\
	...
	At every time interval $\Delta\,t$ we can apply a force on our object in an angle opposite to $\beta$ proportional to our velocity $v$. While this does not give us a equation to work with we can use Euler's method to get a numerical approximation. \todo{Kanske berätta hur luftmotståndsmodellen funkar först (att det är motsat riktningen och = ky), känns lite omotiverat i den här meningen varför man skulle vilja göra det}
	...
	\end{align}
	In order to extrapolate \todo{Är det verkligen extrapolate} $\vec{v}_{drag}$ from $F_D$ we need to consider our objects mass which we will denote $m$. This is due to 
	\begin{align}\nonumber
		\vec{F}=m\vec{a} = m\frac{\Delta\,\vec{v}}{\Delta\,t}\implies \Delta \vec{v} = \frac{\vec{F} \cdot \Delta\,t}{m} = \vec{v}_{drag}
	\end{align}
	This means is that the air resistance is inversely proportional the object's mass. While this might seem counterintuitive at first it is important to remember that a system with a higher mass means that the total energy of the system goes up \todo{of the system increases}. It is therefore only \todo{Tycker inte only behövs} logical that our object travels further with higher mass; there is nowhere else for the extra energy to go. One can also consider conservation of momentum in terms of the air mass.
	\\~\\
	...
	\begin{align}\nonumber
		...
		\end{bmatrix}
	\end{align}
	\todo{Känns som det händer rätt mycket i det där steget}
	...
	\begin{algorithm}[H]
		...
		\end{algorithmic}
	\end{algorithm}
	Using the above algorithm with initial data $v_0 =10,m/s$, $\theta = \ang{45} = \frac{\pi}{4}$, $\rho = 1.225\, kg/m^3$, $A = 1\,m^2$, $m = 1\,kg$, $y_0 = 0\,m$ and $C_D = 0.47$ which corresponds to a heavy beach ball yields the following result. The solid plot is with air resistance and the dashed plot is in a vacuum with the same $v_0$, $y_0$ and $\theta$.
	\begin{center}
		\begin{tikzpicture}
		\begin{axis}[ 
		xlabel=x (m),
		...
		\end{tikzpicture}
	\end{center}
	\todo{Kanske är lite jobbigt, men du borde kanske ha typ en fil per plot som man kan kolla på som läsare}
	...
	\todo{Du börjar bli mindre rigorös, men att det är jobbigt 8och möjligtvis onödigt att vara det hela tiden}
	Consider the simplest case of air resistance; the one-dimensional falling object. Our object does not travel in a parabola \todo{Du skulle kunna säga att det är för att x(t) = 0} and we have previously described this motion as $y(t) = -\frac12gt^2+y_0$. Knowing the air resistance at any time $t$ we can construct a differential equation describing the motion of our object which is as follows \todo{Tycker inte which is utan bara as follows}
	\begin{align}\label{eq:oneDimAirResDiffEq}
		y''(t) &= k\,y'(t)^2 - g \comma k = \frac{\frac12\rho\,A\,C_D}{m}\\\nonumber
		y(0) &= y_0 \comma y'(0) = 0
	\end{align}
	\todo{Ska det inte vara g/m?}
	...
	\end{align}
	$C$ is a constant of integration which is added to $t$ at all times. Let $T = t + C \comma v' = \frac{dv}{dt}$. $G$ can now be written as the following in order to concentrate the unknowns \todo{Aldrig hört concentrate the unknowns formuleringen antar att det betyder att man vill samla dem?}
	\begin{align}\nonumber
	...
	\end{align}
	For simplicity; let $H = \int \tanh(\sqrt{gk}\,t)\,dt, c = \sqrt{gk}, s = c\,t \implies \frac{ds}{dt} = c \implies dt = \frac{ds}{c}$. The integral is now trivial and we can substitute back $s$ after evaluating it.\todo{Istället för det här resonemanget skulle du bara kunna hänvisa till kedjeregeln och därför dela med sqrt(gk). integral(tanh(x)dx) = ln(cosh(x)) => integral(tanh(kx)dx) = ln(cosh(kx))/k}
	\begin{align}\nonumber
	...
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	The slow start with a negative second derivative matches our intuition and at roughly $t = 0.8\,s$ the second derivative appears to approach zero. This should be due to the object reaching its terminal velocity which we can calculate using equation \eqref{eq:oneDimAirResTerminalVel}.
	\begin{align}\nonumber
	...
	\todo{Fast det säger ju ingenting förren du jämfört med verjligheten}
	...
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	Here equation \eqref{eq:tMaxFromHeight} gives $t_{max} = 0.7817\,s$, equation \eqref{eq:oneDimAirResTMax} gives $t_{max} = 0.7830\,s$ \todo{Behövs såhär många värdesiffror?} and the plot gives $t_{max} = 0.7829\,s$. Air resistance plays almost no part here and the dashed plot is nearly indistinguishable from the solid one.
	...
	\begin{align}\nonumber
		...
		u' &= -kvu \comma w' = -kvw-g\,\,\,\,\,
	\end{align}
	where $v = ||\vec{v}|| = \sqrt{u^2+w^2}$ and $\alpha$ is the angle the object is traveling in at an \todo{any?} arbitrary time. This means that $u$ is dependent on $w$ and the other way around. In order to work around this we need to make approximations regarding the relationship between $u$ and $w$ to eliminate $v$ from their differential equations of motion. The following three options are the ways to express $v$ in terms of only one variable.
	\begin{align}\nonumber
		v \approx
		\begin{cases}
			u			&if\,\,\, u \gg w\\
		...
		\end{cases}\\\nonumber
	\end{align}
	\todo{Känns konstigt att du använda mycket mer än och sedan ungefär lika med, vad ska man göra där emellan? Kanske bara borde vara >}
	...
	\end{align}
	We can now plus \todo{plug?} $u$ from equation \eqref{eq:twoDimU} into equation \eqref{eq:twoDimEMOLATW} and solve for $w$.
	\begin{align}\nonumber
	...
	\end{align}
	\todo{\^ tycker det bli lite confusing när du blandar mellan leden så mycket. Asså om a = b förenklar du b och sedan skriver = a och sedan förenklar a osv.}
	...
	\todo{fattar inte riktigt vad som händer w = nånting = blir till någon summa. Du multiplicerar in parantesen, separerar termerna och sen kastar om?}
	\begin{align}\nonumber
	...
	\end{align}
	\todo{vart kommer absarna ifrån?}
	In contrary to our LAT approximation in section \ref{seq:lat} we must now separate the cases where $\hat{w} = \hat{\jmath}, \hat{w} = -\hat{\jmath}$ \todo{är w ens en vektor?} because a negative $w$ should not lead to a positive $u'$ \todo{det gör det ju inte i o m abs?}. When $v$ was approximated as $u$ we assumed that $\hat{u} = \hat{\imath}$ because it is the only realistic scenario. Now that $v$ is approximated as $w$ we need to consider its direction before that context. In the end our equations of motions will consist of two separate $u$ and $w$.
	\todo{Som du kanske märkt så fattar jag inte riktigt ditt resonemang ovan stycke :) }
	\subsubsection{Differential equations of motion}
	...
	For the purpose of this section we will drop the "$+$" postfix \todo{u+ och w+ blir alltså u och w? Kanske skulle kunna göras tydligare}. We begin by solving equation \eqref{eq:twoDimEOMHATWP} in order to be able to express equation \eqref{eq:twoDimEOMHATUP} as a function of one variable \todo{Det är väl bara en variabel t eller menar du bara u eller w? Osäker på om det heter variabler i diff-ekkar}. This is the same as equation \eqref{eq:oneDimAirResGSolve} with the only difference being the sign of $k\,v^2$. It can be shown that using the same technique as that in section \ref{seq:oneDimYt} we can derive equation \eqref{eq:oneDimAirResVT} but with $\tan(\sqrt{gk}\,T)$ instead of $\tanh(\sqrt{gk}\,T)$. From here we can solve for our initial condition $w(0) = w_0$.
	\begin{align}\nonumber
	...
	\end{align}
	For $y$ the procedure is essentially the same. \todo{we perform essentially the same procedure? (inte så stor skillnad men tycker det låter bättre)}
	\begin{align}\nonumber
	...
	\begin{align}\nonumber
		...
		w_{-}(0) &= w_{+}(t_{turn})
	\end{align}
	Here $t_{turn}$ is defined as $w_+(t_{turn}) = \omega\,t_{turn}-\varphi = 0$ i.e. $t_{turn} = \frac{\varphi}{\omega}$. We can introduce the variable $\tau$ and define it as $\tau = t-t_{turn}$ meaning when $t = t_{turn} \comma \tau = 0$ and let $u_{-}$ and $w_{-}$ be functions of $\tau$. \todo{du omdefinierar u- och w- som funktioner av tau? Du kanske borde använda stor U och W t.ex} We begin by solving $u_+(t_{turn})$ and $w_+(t_{turn})$ for our initial conditions.
	\begin{align}\nonumber
	...
	\end{align}
	We can now drop the $-$ and $+$ postfixes and solve equation \eqref{eq:twoDimHATMUFIN} for $x$.\todo{Asså jag är inte helt med på det här med att droppa postfixes. Fattar inte riktigt varför man ska göra det och vad det betyder (att t.ex w- = w så vi kan lika gärna skriva w eller att vi vill studera w- men inte orkar skriva hela så bara skriver w)}
	\begin{align}\nonumber
	...
	\end{align*}
	\todo{Okej tau är en funktion av t. Tycker nästan du borde skriva att t.ex x- är en funktion av t och sedan låta tau vara där, men det kanske bara är min preference.}
	%https://www.desmos.com/calculator/c2hjn8trb7 (x(t))
	%https://www.desmos.com/calculator/krqw78ami5 (y(t))
	\todo{Nice att du kunde återanvända}
	...
	\subsection{Analysis}
	While the discrepancy \todo{använder man discrepancy såhär?} between the experimental result and the analytical model result grew as the height increased the difference stayed negligible but the difference compared to the vacuum model grew. At $8.68\,m$ the vacuum model is no longer a viable option to accurately describe the path of the object. The difference $0.04\,s$ can be partially accounted for by the difficulty in determining the exact time at which the object was dropped. Instead of using a human to drop the object and a human to film the sensor \todo{film the sensor? Film the fall kanske} a better alternative would be a electronically controlled holder connected to a computer and a photosensor at the bottom, also connected to the same computer. The coefficient of drag of a sphere is also hard to determine exactly. Some\cite{c_d} say the coefficient is $0.5$ instead of $0.47$ which decreases the discrepancy for the second fall by roughly $25\,\%$ \todo{Du skulle kunna säga att det inte är en perfect sphere också (har den inte räfflor på sidan och sånt, blir säkert lite deformerad i luften osv.)}. Redoing the experiment in a lab could improve the accuracy as minor winds or unpredictable levels or air density could be avoided. In conclusion it is impossible to say the analytical air resistance model developed has a fundamental error and if any further tweaking were to take place a deeper analysis of the thrown object would be required. The numerical model approached the analytical one in terms of results as $\Delta \rightarrow 0$. \todo{vem hade kunnat trott det? Nejmen du skulle kunna säga att det är för att de är från samma diff.ek}
	\section{Conculsions}
	We have derived the equations of motion for projectile motion and determined some properties of it. These results are testable and exact under the conditions specified. We have developed analytical solutions to one dimensional quadratic air resistance and produced testable predictions which were tried in an experiment and no fundamental errors could be detected. The solutions were developed into two dimensions with approximations to keep the solutions purely analytical. These approximations were compared to numerical solutions and evaluated. While discrepancies could be found they were acceptable in size \todo{beror kanske på vad man ska göra med resultatet :)}.
