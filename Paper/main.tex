...
\setcounter{page}{1}
	...
	\begin{align}\nonumber
		\int-g\,dt = -gt + C
	\end{align}
	Here $C$ is a constant of integration and can be written as $v_0 \sin\theta$ due to $\theta$ being the angle between the $xz$ plane and the $y$ axis making it the initial velocity in the $y$ direction. This allows us to define the total velocity vector $V$.
	\begin{align}\nonumber
		V=
		\begin{bmatrix} 
	...
	\end{align}
	Where $t_{max}$ is the time the object spends in the air. The solution $t=0$ can be discarded as it is the initial value. We can now substitute $t$ for $t_{max}$ in our expression for the $x$ and $z$ positions.
	\begin{align}\nonumber
		\begin{aligned}[t]
		...
		\end{aligned}
	\end{align}
	Let $R$ be the range. By the Pythagorean theorem it is then true that
	\begin{align}\nonumber
	...
	The the optimal theta for maximizing R may be determined by studying the derivative of R with respect to theta.
	\begin{align}\nonumber
		\frac{\partial}{\partial\theta} \frac{v_0^2\sin2\theta}{g} = \frac{2\,v_0^2\cos2\theta}{g}
	\end{align}
	Solving $\frac{\partial R}{\partial \theta} = 0$ gives the maxima of $R$.
	\begin{align}\nonumber
	...
	For the purpose of this section we will not consider the $z$ axis nor the angle $\alpha$ between the $x$ and $z$ axis. The reasoning behind this is that is would introduce unnecessary complexity to the results. Know that these results may be expanded to three dimensions with minimal adjustments for example by multiplying uses of $x$ with $\cos\alpha$.
	...
	\end{align}
	From here we can develop expressions for $\theta$ and $v_0$.
	\begin{align}\nonumber
		\theta &= \frac12\arcsin\Big(\frac{Rg}{v_{0}^2}\Big)\\\nonumber
		v_0 &= \sqrt{\frac{Rg}{\sin2\theta}}
	\end{align}
	...
	\end{align}
	This is a second degree polynomial meaning we can solve it for $h$ using the quadratic formula. Substituting $a$ for $\frac{-gx^2}{2v_{0}^2}$, $b$ for $x$ and $c$ for $-\frac{gx^2}{2v_{0}^2}-y$ gives
	\begin{align}\nonumber
	...
	Consider a parabola where $y(0) > 0$. We will denote this initial location on the $y$ axis $y_0$. We now want to determine which angle $\theta$ yields the farthest possible parabola. We begin by solving $y=0$ for $t_{max}$ where our constant of integration $c_y$ for $y$ is $y_0$.
	\begin{align}\nonumber
		y=v_0\sin\theta\,t_{max} - \frac12gt_{max}^2 + y_0 &= 0 \implies\\\nonumber
		-\frac12g\,t_{max}^2+v_0\sin\theta\,t_{max}+y_0 &= 0
	\end{align}
	Using the quadratic formula and substituting $a$ for $-\frac12g$, $b$ for $v_0\sin\theta$ and $c$ for $y_0$ we obtain the following expression for $t_{max}$
	\begin{align}\nonumber
	...
	\end{align}
	Setting the second term as negative yields a negative time since $y_0$ is defined as $y_0 > 0$. This is a representation of the object traveling in a parabola at all times $t$ and crossing the initial position at $t=0$. We will therefore write it as positive from now on. We can now find the length of a parabola and see how it varies with a changing angle $\theta$. Let $R$ denote the range of the projectile given by $x(t_{max})$.
	\begin{align}\nonumber
	...
	\end{align}
	Maximizing $R$ by finding an optimal $\theta$ is not easily solvable algebraically. We will therefore move on to numerical methods. 
	\subsection{Algorithm}
	We want to write an algorithm that takes the initial height and velocity and returns the optimal angle. The code tests as many as possible angles $\theta$ in our equation for $R$ and determines which gives the farthest possible throw. The procedure is described in the following pseudo code.
	\begin{algorithm}[H]
	...
	Air resistance is in some way proportional to the velocity of the object and directed in the direction opposite to the current movement. For our algorithm this means that at every time interval $\Delta\,t$ we want to apply a force on our object in an angle opposite to $\beta$ in some way proportional to our velocity $v$.
	...
	\end{align}
	In order to determine $\vec{v}_{drag}$ from $F_D$ we need to consider our objects mass which we will denote $m$. This is due to 
	\begin{align}\nonumber
		\vec{F}=m\vec{a} = m\frac{\Delta\,\vec{v}}{\Delta\,t}\implies \Delta \vec{v} = \frac{\vec{F} \cdot \Delta\,t}{m} = \vec{v}_{drag}
	\end{align}
	This means is that the air resistance is inversely proportional the object's mass. While this might seem counterintuitive at first it is important to remember that a system with a higher mass means that the total energy of the system increases. It is therefore logical that our object travels further with higher mass; there is nowhere else for the extra energy to go. One can also consider conservation of momentum in terms of the air mass.
	\\~\\
	...
	\end{align}
	For simplicity; let $H = \int \tanh(\sqrt{gk}\,t)\,dt \comma c = \sqrt{gk} \comma s = c\,t \implies \frac{ds}{dt} = c \implies dt = \frac{ds}{c}$. The integral is now trivial and we can substitute back $s$ after evaluating it.
	\begin{align}\nonumber
	...
	\end{align}
	This increase of approximately $30\,\%$ matches our physical intuition for such a large object and adds credit to the accuracy of equation \eqref{eq:oneDimAirResTMax}.
	...
	\begin{align}\nonumber
		...
		\begin{cases}
			u			&\textrm{if}\,\,\, u \gg w\\
			w			&\textrm{if}\,\,\, w \gg w\\
			\sqrt{2}u	&\textrm{if}\,\,\, u \approx w\\
		\end{cases}\\\nonumber
	\end{align}
	We treat these as three separate cases and label them as LAT, HAT and SAT for Low Angle Trajectory, High Angle Trajectory and Split Angle Trajectory respectively. At times none of these approximations may be applicable which is their weakness. Additional cases such as $v \approx \sqrt{5}u \,\, \textrm{if} \,\, u \approx 2w$ could be considered but their derivation would be nearly identical to that of $v \approx \sqrt{5}u$.
	...
	\end{align}
	We can now plug $u$ from equation \eqref{eq:twoDimU} into equation \eqref{eq:twoDimEMOLATW} and solve for $w$.
	\begin{align}\nonumber
	...
	\end{align}
	In contrary to our LAT approximation in section \ref{seq:lat} we must now separate the cases where $w \geq 0\comma w < 0$ because a negative $w$ should not lead to a positive $u'$ and we can not solve differential equation containing an absolute value. When $v$ was approximated as $u$ we assumed that $u > 0$ because it is the only realistic scenario. Now that $v$ is approximated as $w$ we need to consider its direction first. In the end our equations of motions will consist of two separate $u$ and $w$.
	\subsubsection{Differential equations of motion}
	...
	For the purpose of this section we will drop the "$+$" postfix since we only consider $u_{+}$ and $w_{+}$. We begin by solving equation \eqref{eq:twoDimEOMHATWP} in order to be able to express equation \eqref{eq:twoDimEOMHATUP} as a function of one variable. This is the same as equation \eqref{eq:oneDimAirResGSolve} with the only difference being the sign of $k\,v^2$. It can be shown that using the same technique as that in section \ref{seq:oneDimYt} we can derive equation \eqref{eq:oneDimAirResVT} but with $\tan(\sqrt{gk}\,T)$ instead of $\tanh(\sqrt{gk}\,T)$. From here we can solve for our initial condition $w(0) = w_0$.
	\begin{align}\nonumber
	...
	\end{align}
	Having solved for $w$ we can now solve equation \eqref{eq:twoDimEOMHATUP} for $u$. To shorten equation \eqref{eq:twoDimEOMHATWPSOLVE} we let $\omega = \sqrt{gk} \comma \varphi = \arctan\big(w_0\sqrt{\frac{k}{g}}\big)$. We can simplify the constants as $-k\sqrt{\frac{g}{k}} = -\omega$.
	\begin{align}\nonumber 
	...
	\end{align}
	For $y$ we perform essentially the same procedure.
	\begin{align}\nonumber
